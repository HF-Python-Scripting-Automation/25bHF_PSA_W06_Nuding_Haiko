import logging
from psa_utils.logger import get_logger

def get_aufgabe_01_logger():
    return get_logger("Aufgabe-01", filename="aufgabe-01.log", level=logging.DEBUG, clear=True)

def get_aufgabe_02_logger():
    return get_logger("Aufgabe-02", filename="aufgabe-02.log", level=logging.DEBUG, clear=True)

def get_aufgabe_03_logger():
    return get_logger("Aufgabe-03", filename="aufgabe-03.log", level=logging.DEBUG, clear=True)

def get_aufgabe_04_logger():
    return get_logger("Aufgabe-04", filename="aufgabe-04.log", level=logging.DEBUG, clear=True)

def get_aufgabe_05_logger():
    return get_logger("Aufgabe-05", filename="aufgabe-05.log", level=logging.DEBUG, clear=True)

def get_aufgabe_06_logger():
    return get_logger("Aufgabe-06", filename="aufgabe-06.log", level=logging.DEBUG, clear=True)

def get_aufgabe_07_logger():
    return get_logger("Aufgabe-07", filename="aufgabe-07.log", level=logging.DEBUG, clear=True)

def get_aufgabe_08_logger():
    return get_logger("Aufgabe-08", filename="aufgabe-08.log", level=logging.DEBUG, clear=True)

def get_aufgabe_09_logger():
    return get_logger("Aufgabe-09", filename="aufgabe-09.log", level=logging.DEBUG, clear=True)

def get_aufgabe_10_logger():
    return get_logger("Aufgabe-10", filename="aufgabe-10.log", level=logging.DEBUG, clear=True)

def get_aufgabe_11_logger():
    return get_logger("Aufgabe-11", filename="aufgabe-11.log", level=logging.DEBUG, clear=True)

def get_aufgabe_12_logger():
    return get_logger("Aufgabe-12", filename="aufgabe-12.log", level=logging.DEBUG, clear=True)

def get_aufgabe_13_logger():
    return get_logger("Aufgabe-13", filename="aufgabe-13.log", level=logging.DEBUG, clear=True)
